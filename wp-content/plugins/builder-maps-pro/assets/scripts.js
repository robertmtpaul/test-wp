...
(function ($, Themify) {
    ...
    resolve_address = async function (address) {
        ...
        if (Array.isArray(position)) {
            ...
            return new Promise(function(address,resolve, reject) {
        ...
        for(let i=items.length-1;i>-1;--i){
            ...
            let config = {
               ...
               // node.data('gmap_object', map);
				...
				then(function(map,poly,item,position){
					if(position){
						map.setCenter(position);
						for(var k=markers.length-1;k>-1;--k){
							Themify.requestIdleCallback(function(map,poly,m){
								resolve_address(m.getAttribute('data-address')).then(function(map,poly,m,pos){
									if(pos){
								...
								}.bind(null,map,poly,m));
							}.bind(null,map,poly,markers[k]),400);
						}
					}
				}.bind(null,map,poly,item));
    ...
    Themify.on( 'builder_load_module_partial',function(el,type,isLazy){
        ...
        Themify.requestIdleCallback(function(){
            if (isInit===null && (!window['google'] || typeof window['google'].maps !== 'object')) {
                ...
                Themify.LoadAsync('//maps.googleapis.com/maps/api/js', function(el){
                        if(isInit===true){
                                Init(el);
                        }
                        else{
                                Themify.on('themify_google_map_pro_loaded',Init.bind(null,el),true);
                        }
                }.bind(null,el), 'v=3.exp&callback=ThemifyMapsProInit&key=' + themify_vars.map_key, null, function () {
