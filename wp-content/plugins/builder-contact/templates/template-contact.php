...
<div <?php echo self::get_element_attributes(self::sticky_element_props($container_props,$fields_args)); ?>>
	...
	<form
		...
		data-orig-id="<?php echo esc_attr( get_the_ID() ); ?>"
	>
	...
	$field['label'] = isset( $field['label'] ) ? $field['label'] : '';
			...
			<div class="control-input">
				...
				<textarea name="field_extra_<?php echo $index; ?>" id="field_extra_<?php echo $args['module_ID'] . '_' . $index; ?>"<?php if($isAnimated===false):?> placeholder="<?php echo esc_html($field['value']); ?>"<?php endif;?> rows="8" cols="45" class="form-control" <?php echo $required===true?  'required' : '' ?> ></textarea>
			    <?php elseif( 'text' === $field['type'] ||  'tel' === $field['type'] || 'upload' === $field['type']): ?>
				<input type="<?php echo $field['type']==='upload'?'file':$field['type']?>" name="field_extra_<?php echo $index; ?>" id="field_extra_<?php echo $args['module_ID'] . '_' . $index; ?>"<?php if($isAnimated===false &&  'upload' !== $field['type']):?> placeholder="<?php echo esc_html($field['value']); ?>"<?php endif;?> class="form-control" <?php echo $required===true?  'required' : '' ?> />
				...
				<?php elseif( 'select' === $field['type'] ): ?>
				    <select id="field_extra_<?php echo $args['module_ID'] . '_' . $index; ?>" name="field_extra_<?php echo $index; ?>" class="form-control" <?php echo $required===true?  'required' : '' ?>>
					    <?php if($required===false):?><option value=""><?php _e('Please select one' , 'builder-contact')?></option><?php endif;?>
					<?php foreach( $field['value'] as $value ): ?>
	...
	<?php endforeach;?>
	    ...
	    <?php endif; ?>
			    ...
			    <?php if ( $fields_args['field_optin_active'] ) : ?>
				    ...
				    $optin_instance = method_exists('Builder_Optin_Service', 'get_providers')?Builder_Optin_Service::get_providers( $fields_args['provider'] ):Builder_Optin_Services_Container::get_instance()->get_provider($fields_args['provider']);
					$optin_inputs='';
				    if($optin_instance){
